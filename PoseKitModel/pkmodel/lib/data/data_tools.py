...
def label2heatmap(keypoints, other_keypoints, img_size):
    ...
    if keypoints_range.size == 0:
        heatmaps = np.zeros((len(keypoints)//3, img_size//4, img_size//4), dtype=np.float32)
        return heatmaps, 3
    ...
    for i in range(0,len(keypoints),3):
        ...
        heatmaps.append(heatmap * weight)
...
class TensorDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        item = copy.deepcopy(self.data_labels[index])
        ...
        kps_mask = np.ones(len(keypoints)//3, dtype=np.float32)
        for i in range(len(keypoints)//3):
            v = float(keypoints[i*3+2])
            if v <= 0:
                kps_mask[i] = 0.0
            else:
                kps_mask[i] = min(max(v, 0.0), 1.0)
...
def getDataLoader(mode, input_data, cfg):
    ...
    if mode=="trainval":
        data_aug = DataAug(cfg['img_size']) if cfg.get('use_data_aug', True) else None
